...
class SoftwareController {
  ...
  Future<String> registrarSoftware(
    BuildContext context, String idEquipoComputo, String nombre, String fabricante, String version, String tipoLicencia, String licenciaClave, String urlImagen, {bool editar = false, int id=0}) async {
    try {
      if (id>0) {
        await supabase.from('SOFTWARE').update({
          ...
          'LICENCIA_CLAVE': licenciaClave,
          'URL_IMAGEN': urlImagen,
        }).match({'ID': id}).then((value) async {
          log('Software actualizado: $value');
        });
      } else {
  ...
  Future<List<Software>> getbyIdActivo(
      String? idActivo,
      ) async {
    try {
       final data = await supabase
              ...
              as List<dynamic>;
          return (data).map((e) => Software.fromMap(e)).toList();
  ...
  Future<String> eliminarbyid(context, String id) async {
    try {
      final data =
          await supabase.from('SOFTWARE').delete().match({'ID': id});
      log('Eliminando:$data');
      return 'ok';
    }  on PostgrestException catch (errorPostgres) {
      var error = Utilidades().validarErroresEliminar(
          errorPostgres.code!, 'Este activo',
          objetoLlaveForaneo: 'software');
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(
          error,
          style: FlutterFlowTheme.of(context).bodyText2.override(
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(
          'Ha ocurrido un error inesperado al intentar eliminar, verfique su conexión a internet',
          style: FlutterFlowTheme.of(context).bodyText2.override(
                fontFamily: FlutterFlowTheme.of(context).bodyText2Family,
                color: FlutterFlowTheme.of(context).tertiaryColor,
                useGoogleFonts: GoogleFonts.asMap()
